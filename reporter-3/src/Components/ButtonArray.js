...
const ButtonArray = ({ toggleData, isDataShowing }) => {
	...
	function unloadPlaceGunObject() {
		Object.keys(placeGunObject).forEach(key => {
			while (placeGunObject[key].length) {
				todayDocRef
					.update({
						[key]: firestore.FieldValue.arrayUnion(
							placeGunObject[key][0]
						)
					})
	...
	return (
		<div className="buttonArray">
			<button onClick={toggleData} id="show-data">
				{isDataShowing ? (
					<Link to="/reporter-3/">Hide data</Link>
				) : (
					<Link to="/reporter-3/today">Show data</Link>
				)}
			</button>
			<button onClick={handleClick} name="😃" id="laugh">
				<span role="img" aria-label="laugh">
					😃
				</span>
			</button>
			<button onClick={handleClick} name="😊" id="warm">
				<span role="img" aria-label="warm">
					😊
				</span>
			</button>
			<button onClick={handleClick} name="😘" id="kiss">
				<span role="img" aria-label="kiss">
					😘
				</span>
			</button>
			<button onClick={handleClick} name="👎" id="thumbs-down">
				<span role="img" aria-label="thumbs down">
					👎
				</span>
			</button>
			<button onClick={handleClick} name="❓" id="question">
