...
const MultiDayChart = ({
	...
	const totalToday = Object.keys(todayDataObject).reduce(
		(sum, emoji) => {
			...
			endObj[emoji] = [
				{
					...
					percent:
						Math.floor(
							(todayDataObject[emoji].length / totalToday) * 100
						) || 0
				}
			]
	...
	function getLastDocRefs(lastDateStrings) {
		return lastDateStrings.map(dateString =>
			db.doc(
				`users/MPQ/${dateString.slice(-4)}/${dateString.slice(
					0,
					3
				)} ${dateString.slice(-4)}/dailys/${dateString}`
			)
		)
	}
	...
	function prepLastDataObject(lastDateStrings) {
		let lastDataObject = lastDateStrings.reduce(
			(endObj, dateString) => {
				endObj[dateString] = 0
				return endObj
			},
	...
	async function parseLastData(
		lastDateStrings,
		lastDocRefs,
		dataObject
	) {
		for (let i = 0; i < lastDocRefs.length; i++) {
			await lastDocRefs[i].get().then(doc => {
		...
		const totalsParsed = Object.keys(dataObject).reduce(
			(endObj, date) => {
				endObj[date] = dataObject[date]
				endObj[date]["totalReports"] = Object.keys(
					endObj[date]
				).reduce((sum, emoji) => {
		...
		const parsedObject = ["😘", "😊", "😃", "👎", "❓"].reduce(
			(endObj, emoji) => {
				endObj[emoji] = Object.keys(dataObject).reduce(
					(endArray, key) => {
						endArray = [
							...endArray,
							{
								...
								count:
									Math.floor(
										(dataObject[key][emoji].length /
											totalsParsed[key]["totalReports"]) *
											100
									) || 0
							}
						]
				...
				<VictoryChart domainPadding={{ x: [0, 10], y: 10 }}>
					<VictoryAxis
						style={{
							...
							tickLabels: { padding: 12, fontSize: 10, angle: -30 }
						}}
					...
					{[
						...
						<VictoryLine
							...
							style={{
								data: { stroke: pair.stroke, strokeWidth: "4px" },
